    ...
    public class CitiesRepository: ICitiesRepository
    {
        ...
        public async Task InsertAllAsync(string staticFilePath)
        {
            ...
            using (var reader = new StreamReader(File.OpenRead(staticFilePath)))
            {
                ...
                var result = await client.IndexManyAsync(cites, IndexName);
                Console.WriteLine($"added {cites.Count} documents with {result.IsValid} error: {result.ItemsWithErrors.FirstOrDefault()?.Error}");
            }
        }
    }
}